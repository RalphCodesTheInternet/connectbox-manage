...
get.clientwifiscan = function () {
	...
	var scanData = execute(`sudo iw ${clientWifiInterfaceName} scan`);

	var block = scanData.split(clientWifiInterfaceName);

	var response = [];

	for (var i = 0; i < block.length; i++) {
		var wifiItem = {};
		if (block[i].indexOf('SSID:') !== -1) {
			var ssid = block[i].substring(block[i].indexOf('SSID:') + 6, block[i].indexOf('\n', block[i].indexOf('SSID:')));
			if (ssid !== '') {
				wifiItem.ssid = ssid;
				wifiItem.encryption = 'none'
				if (block[i].indexOf('WPA:') !== -1) {
					wifiItem.encryption = 'WPA'
				}
				if (block[i].indexOf('WPS:') !== -1) {
					wifiItem.encryption = 'WPS'
				}
				if (block[i].indexOf('WPS:') !== -1 && block[i].indexOf('WPA:') !== -1) {
					wifiItem.encryption = 'WPA/WPS'
				}
				response.push(wifiItem)
			}
		}
	}
	return response;
